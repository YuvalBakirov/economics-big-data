...
4. base: This column includes a code representing the Uber base or the specific area/company that managed the ride (License Number).
...
Base -  the TLC (Taxi and Limousine Commission) base company code affiliated with the Uber pickup. This code identifies the licensed dispatching base that managed the Uber trip. Every ride must be affiliated with a TLC-licensed base, which is responsible for dispatching the trip.
...
```{r adding_is_weekend}
# Function to add 'is_weekend' and 'day_of_week' features
...
* Tree-based models can handle categorical variables without needing to transform them, making this representation ideal when using such models.
...
* Linear models assume a linear relationship between features and the target variable. If you leave time_of_day as 1, 2, 3, 4 (categorical), the model might falsely interpret that higher values imply higher importance, which isn’t true. Dummy variables prevent this by representing each category as its own independent feature (1 or 0), ensuring that the model does not assume any ordinal relationship.
* Dummy variable trap: When using one-hot encoding, it's common to drop one category (here, is_night) because its value can be inferred when all other dummy variables are 0. This avoids redundancy and multicollinearity.
...
By adding both a categorical feature and dummy variables for time_of_day, later on we will switch between the two versions depending on the model we will use:
* Categorical feature (time_of_day_cat) for tree-based models.
* Dummy variables (is_morning, is_afternoon, etc.) for linear models.
...
We have confirmed that the weather data is recorded at exact hourly intervals, meaning there are no variations in the minutes or seconds. Therefore, when merging this data with our Uber pickup data, we'll need to align our timestamps accordingly. Specifically, we will round our pickup timestamps to the nearest hour to ensure consistency and enable a seamless and accurate merging process. This alignment helps maintain the integrity of our analysis by matching the time intervals precisely.

This code combines weather data (temperature, humidity, wind speed, and snow indication) with the Uber pickup data (1000m and 2000m datasets) using the timestamp as the key. If the minute value in the Uber pickup data (%M) is greater than 30, it uses the weather data from the next hour. Otherwise, it uses the weather data from the same hour.

We are using a function called rounded_timestamp that adds a new feature, rounded_timestamp, to the Uber pickup datasets. This feature rounds the timestamp either up to the next hour or down to the start of the current hour based on the minute value (%M). This allows us to accurately align and merge the weather data with the Uber pickup data based on the adjusted timestamps.
```{r}
# Function to adjust the timestamp based on the minute
rounded_timestamp <- function(df) {
  df %>%
    mutate(
      rounded_timestamp = case_when(
        minute(timestamp) > 30 ~ as.POSIXct(floor_date(timestamp, "hour") + hours(1), format="%Y-%m-%d %H:%M:%S", tz = "UTC"),
        TRUE ~ as.POSIXct(floor_date(timestamp, "hour"), format="%Y-%m-%d %H:%M:%S", tz = "UTC")
      )
    )
}
```
