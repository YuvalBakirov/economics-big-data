...
train_filtered_1000m_data <- create_time_intervals(train_filtered_1000m_data)
...
train_filtered_2000m_data <- create_time_intervals(train_filtered_2000m_data)
...
```{r}
plot_top_10_pickup_intervals <- function(data, title, fill_color) {
  ...
  ggplot(top_10_pickups, aes(x = time_interval, y = count_pickups_per_interval)) +
    geom_bar(stat = "identity", fill = fill_color, width = 0.6) + # Adjust width for spacing
    labs(title = title,
         x = "Time Interval",
         y = "Pickup Counts") +
    theme_minimal() +
...
check_lat_lon_base <- function(data) {
  ...
  if (nrow(invalid_data) > 0) {
    cat("Invalid latitude, longitude, or missing base found:\n")
    print(invalid_data)
  } else {
    cat("All lat, lon, and base values are valid.\n")
  }
}
```
...
# Check for the 1000m data
cat("For 1000m data:\n")
check_lat_lon_base(train_filtered_1000m_data)
```
```{r}
# Check for the 2000m data
cat("For 2000m data:\n")
check_lat_lon_base(train_filtered_2000m_data)
```
...
# Function to add 'time_of_day' dummy features
add_time_of_day <- function(data) {
  data %>%
    mutate(
      hour = hour(timestamp),
...
```{r}
# Apply the function to the 1000m dataset
train_filtered_1000m_data <- add_time_of_day(train_filtered_1000m_data)
head(train_filtered_1000m_data)
```
```{r}
# Apply the function to the 2000m dataset
train_filtered_2000m_data <- add_time_of_day(train_filtered_2000m_data)
head(train_filtered_2000m_data)
```
