...
4. Handling Missing Values (NAs):
* Number of persons killed and is snowing last hour both show a large number of missing values (NAs).
...
1. Number of Pickups:
* Negative correlation with is_night (-0.33) suggests fewer pickups occur during nighttime.
...
4. Handling Missing Values (NAs):
* Similar to the 1000m data, features like number_of_persons_killed and is_snowing_last_hour have many missing values (NAs).
We will deal with it later.
...
```
As you can see, the number of person killed is very negligible in both tables. In the 1000 meter radius table, there are no persons killed at all from April to mid-September, therefore it is very likely that there will be no persons killed until the end of September.
...
model <- lm(number_of_pickups ~ temp + humidity + wind_speed + feels_like + clouds_all + is_raining_last_hour + is_night + is_weekend+ number_of_persons_injured+ number_of_crashes+ event_count+ taxis_pickup_count+ total_arrests+ memorial_day+ independence_day+labor_day , data = train_filtered_1000m_data)
...
```{r}
# Define a function to fit the linear model and calculate VIF
calculate_vif <- function(data, dataset_name) {
  # Fit a linear model to calculate VIF
  model <- lm(number_of_pickups ~ temp + humidity + wind_speed + feels_like + clouds_all + 
              is_raining_last_hour + is_night + is_weekend + number_of_persons_injured + 
              number_of_crashes + event_count + taxis_pickup_count + total_arrests + 
              memorial_day + independence_day + labor_day, data = data)
...
```{r}
# Call the function for train_filtered_1000m_data
calculate_vif(train_filtered_1000m_data, "train_filtered_1000m_data")
```
```{r}
# Call the function for train_filtered_2000m_data
calculate_vif(train_filtered_2000m_data, "train_filtered_2000m_data")
```

todo explain for both datasets and change numbers
...
columns_to_remove <- c("feels_like", "is_snowing_last_hour", "number_of_persons_killed", "humidity", "clouds_all", "wind_speed")
