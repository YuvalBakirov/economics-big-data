...
Note: all the features are numeric (0/1 or a number) except for time_interval (date)
...
create_corr_matrix <- function(data, dataset_name) {
  # Select all numeric columns, excluding 'time_interval'
  numeric_data <- data %>%
    select_if(is.numeric)  # Select numeric columns, exclude non-numeric ones
...
```{r}
# Function to print feature pairs and their correlation values
print_corr_pairs <- function(data, dataset_name) {
  ...
  # Replace NaNs or Inf values with NA
  cor_matrix[is.na(cor_matrix)] <- NA
  
  # Print all feature pairs with their correlation
  feature_names <- colnames(cor_matrix)
  cat("Correlation Pairs for", dataset_name, ":\n")
  for (i in 1:(ncol(cor_matrix) - 1)) {
    cat("Correlation with:", feature_names[i], ":\n")
    for (j in (i + 1):ncol(cor_matrix)) {
      cat(paste(feature_names[i], "and", feature_names[j], ":", cor_matrix[i, j], "\n"))
    }
    cat("\n")
  }
}
...
create_corr_matrix(train_filtered_1000m_data, "1000m Data")
```
```{r}
# print feature pairs and their correlation for train_filtered_1000m_data:

print_corr_pairs(train_filtered_1000m_data, "1000m Data")
```

We can see that for the 1000m data:
1. Number of Pickups:
* Negative correlation with is_weekend (-0.35) and is_night (-0.50), meaning fewer pickups occur during weekends and nights.
* Positive correlation with taxis_pickup_count (0.31), indicating more taxi pickups are associated with higher numbers of regular pickups.
* Moderate correlation with misdemeanor_count (0.27) and total_arrests (0.30), which could suggest that increased law enforcement activity might correlate with more pickups in the area.

2. Weather Factors:
* Temperature has a strong positive correlation with feels_like (0.99) as expected, but also has a negative correlation with clouds_all (-0.30) and is_raining_last_hour (-0.25). This suggests that higher temperatures are typically associated with fewer clouds and less rain.
* Humidity has a strong positive correlation with clouds_all (0.61) and is_raining_last_hour (0.56), indicating that higher humidity levels are associated with cloudier and rainier conditions.

3. Event Count:
* A moderate positive correlation is seen between event_count and temp (0.24) and feels_like (0.23), suggesting that events might tend to occur during warmer weather.

4. Law Enforcement:
* Felony Count is strongly positively correlated with total_arrests (0.56), which is expected as felony counts would contribute to overall arrests.
* Misdemeanor Count has a very strong positive correlation with total_arrests (0.96), indicating that misdemeanors make up a significant portion of arrests.

5. Handling Missing Values (NAs):
* Number of persons killed and is snowing last hour both show a large number of missing values (NAs). This likely means that there were no instances of these events occurring in the dataset, which would lead to the absence of correlations. These features are therefore not expected to explain or predict the number of pickups due to their lack of variation in the data.


Conclusions:
1. Determining Less Relevant Features for Predicting Number of Pickups: Features that have low correlation with number_of_pickups are less likely to be useful for predicting the number of pickups. Based on the data, these include:
* Wind_speed (-0.02): The correlation is very close to zero, suggesting wind speed has minimal impact on the number of pickups.
* Memorial_day (-0.08), Independence_day (-0.09), and Labor_day (-0.05): These holiday variables have weak correlations with the number of pickups and likely do not have a strong impact on pickup activity.
* Felony_count (0.23) and Misdemeanor_count (0.27): While there is some correlation, it is not particularly strong, meaning that law enforcement activity may not be a primary factor in predicting pickups.

2. Identifying Potential Multicollinearity: There are several instances of high correlation between features that indicate potential multicollinearity:
* Temperature and feels_like (0.99): These variables are almost perfectly correlated, indicating strong multicollinearity. Since these variables essentially represent the same information, one of them (likely feels_like) can be removed from a predictive model to prevent redundancy.
* Misdemeanor_count and total_arrests (0.96): These variables are also highly correlated, indicating that misdemeanors contribute substantially to total arrests. Including both in a model could cause instability, so one of these variables (e.g., misdemeanor_count) could be removed to reduce multicollinearity.
* Humidity and is_raining_last_hour (0.56): Higher humidity is associated with recent rainfall. While not as high as other correlations, this could also indicate some redundancy in the information provided by these two variables.

```{r}
# plot corr mat for train_filtered_2000m_data:
create_corr_matrix(train_filtered_2000m_data, "2000m Data")
```

```{r}
# print feature pairs and their correlation for train_filtered_1000m_data:
print_corr_pairs(train_filtered_2000m_data, "2000m Data")
```

We can see that for the 2000m data:
1. Number of Pickups:
* Negative correlation with is_night (-0.33) suggests fewer pickups occur during nighttime, although less strongly than in the 1000m data.
* Positive correlation with taxis_pickup_count (0.47), indicating that higher taxi pickups are strongly associated with regular pickups.
* Moderate positive correlations with event_count (0.37) and number_of_crashes (0.37), suggesting that events and traffic incidents might be associated with higher numbers of pickups.

2. Weather Factors:
* Temperature has a strong positive correlation with feels_like (0.99) as expected, but a negative correlation with clouds_all (-0.31) and 
* is_raining_last_hour (-0.25). This indicates that warmer weather typically corresponds with fewer clouds and less rain.
* Humidity has a strong positive correlation with clouds_all (0.61) and is_raining_last_hour (0.56), showing a relationship between higher humidity and cloudier or rainier conditions, similar to the 1000m data.

3. Event Count:
* Moderate positive correlation with temp (0.25) and feels_like (0.24), indicating that events tend to occur in warmer weather.
* Moderate positive correlation with number_of_crashes (0.26) suggests that events are potentially associated with traffic incidents, possibly due to congestion.

4. Law Enforcement:
* Felony Count has a strong positive correlation with total_arrests (0.92) and misdemeanor_count (0.82), indicating that both felony and misdemeanor offenses contribute significantly to total arrests.
* Misdemeanor Count has an extremely high positive correlation with total_arrests (0.98), showing that misdemeanors make up the bulk of arrests.

5. Handling Missing Values (NAs):
* Similar to the 1000m data, features like number_of_persons_killed and is_snowing_last_hour have many missing values (NAs), indicating that these events were either rare or absent in the dataset, making them less likely to contribute meaningfully to predicting pickups.


Conclusions:
1. Determining Less Relevant Features for Predicting Number of Pickups: Based on the 2000m data, the following features have low correlation with number_of_pickups and are less likely to be important predictors:
* Wind_speed (-0.03): Very low correlation with pickups, suggesting it has minimal impact on pickup activity.
* Memorial_day (-0.08), Independence_day (-0.05), and Labor_day (-0.01): These holidays show weak correlations and likely do not have a strong effect on pickup numbers.
* Felony_count (0.13) and Misdemeanor_count (0.18): These law enforcement variables have moderate but not particularly strong correlations with pickups, suggesting they may not be key predictors.

2. Identifying Potential Multicollinearity: Several features exhibit high correlation, indicating potential multicollinearity:
* Temperature and feels_like (0.99): Almost perfect correlation, indicating redundancy. Removing one of these (e.g., feels_like) would help reduce multicollinearity.
* Misdemeanor_count and total_arrests (0.98): Since misdemeanors dominate total arrests, it may be redundant to include both in the model. Removing one of these features would simplify the model.
* Felony_count and total_arrests (0.92): Similarly, felonies also contribute significantly to total arrests, and removing one of these features could improve model stability.
* Humidity and clouds_all (0.61): This moderate correlation suggests some redundancy in these atmospheric variables, and removing one could improve interpretability without significant information loss.


What about NAs:
The NA values in the is_snowing_last_hour & number_of_persons_killed of the correlation matrix occur because the is_snowing_last_hour & number_of_persons_killed feature likely contains only a constant value (like all zeros), resulting in a standard deviation of zero. Correlation cannot be computed if there is no variability in the data, which leads to NA in the correlation matrix.
...
Constant Feature: If all values in is_snowing_last_hour & number_of_persons_killed are NA, R cannot compute a correlation, resulting in NA in the correlation matrix.
...
is_snowing_last_hour (this is for both 1000m & 2000m datasets):
```{r}
...
```
We observed that all values for the 'is_snowing_last_hour' feature in both the 1000m and 2000m datasets are constant (0).
This makes sense given that the months in the data (April to September) are not snowy months in New York City. 
Typically, snow occurs in the winter months of January and February, so it is reasonable to see no snow-related data during the spring and summer months covered by the dataset.
Therefore, We will omit this variable later for both datasets.
...
# Remove the columns from test_fixed_data
test_fixed_data <- remove_columns(test_fixed_data, columns_to_remove)
...
cat("\n")
print(colnames(test_fixed_data))
...
humidity: Humidity on its own, while affecting comfort, does not have a strong direct link to transportation choices like Uber. It is less noticeable for people compared to rain or temperature. Moreover, in moderate ranges, it might not significantly impact decision-making for taking an Uber.
...
clouds_all: Cloud cover may have some impact on comfort, but it lacks a direct influence on mobility decisions. People are not as likely to alter their transportation plans based on clouds alone unless they are associated with imminent rain, which is already captured by the "is_raining_last_hour" feature.

Features to Keep:
temp (Temperature): Temperature has a clear and direct impact on transportation choices. In both extreme heat and cold, people are less willing to walk or use public transit. In New York, uncomfortable temperatures, whether very hot or very cold, can drive people to opt for Uber rides. Hence, it is a strong predictor of pickup demand.
...
```{r}
# Columns to be removed
columns_to_remove <- c("clouds_all", "wind_speed", "humidity")
...
Temperature: The ranges and averages for this feature are nearly identical to the 1000m dataset, reflecting similar environmental conditions.
