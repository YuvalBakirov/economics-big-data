...
```
As seen in the two graphs, there are significant amounts of data outside the desired range of 18:00 to 00:00 for both datasets (1000m and 2000m data). Therefore, to focus on the data for pickups that occurred between 18:00 and 00:00, we will apply a filter to exclude the hours outside this range.

Filter data to include only pickups between 18:00 and 00:00:
```{r}
# For 1000-meter filtered data
train_filtered_1000m_data <- train_filtered_1000m_data %>%
...
```{r}
nrow(train_filtered_1000m_data)
nrow(train_filtered_2000m_data)
```
...
start_date <- as.POSIXct("2014-04-01 18:00:00")
end_date <- as.POSIXct("2014-09-9 23:59:59")

# Generate a sequence of 15-minute intervals between start_date and end_date
intervals <- seq(start_date, end_date, by = "15 mins")

# Create a data frame from the intervals
intervals_df <- data.frame(Interval = intervals)

# Filter only the time between 18:00 and 00:00 (excluding 00:00)
intervals_df <- intervals_df %>%
  filter(format(Interval, "%H:%M:%S") >= "18:00:00" & format(Interval, "%H:%M:%S") < "24:00:00") %>%
  filter(format(Interval, "%H:%M:%S") != "00:00:00")

# Extract unique days
unique_days_df <- intervals_df %>%
  mutate(Unique_Day = as.Date(Interval)) %>%
  distinct(Unique_Day)

# View the unique days DataFrame
unique_days_df
```

```{r}
# Count the number of unique days
unique_days_count <- nrow(unique_days_df)
print(paste("Total number of unique days:", unique_days_count))
```
...
```{r}
# Define the generic function to find and print missing dates
find_missing_days <- function(expected_intervals_df, actual_data_df, interval_col_expected, interval_col_actual) {
  # Extract unique days from the expected intervals DataFrame
  unique_days_expected <- expected_intervals_df %>%
    mutate(Unique_Day = as.Date(!!sym(interval_col_expected))) %>%
    distinct(Unique_Day)
  
  # Extract unique days from the actual data DataFrame
  unique_days_actual <- actual_data_df %>%
    mutate(Unique_Day = as.Date(!!sym(interval_col_actual))) %>%
    distinct(Unique_Day)
  
  # Find the days that are in expected intervals but missing in actual data
  missing_days <- setdiff(unique_days_expected$Unique_Day, unique_days_actual$Unique_Day)
  print(unique_days_expected)
  print(unique_days_actual)
...
# Example usage:
find_missing_days(intervals_df, pickup_counts_1000m, "Interval", "time_interval")

```


```{r}
# Define the generic function to find and print missing date-time intervals
find_missing_datetime_intervals <- function(expected_intervals_df, actual_data_df, interval_col_expected, interval_col_actual) {
  # Extract unique date-time intervals from the expected intervals DataFrame
  unique_intervals_expected <- expected_intervals_df %>%
    mutate(Unique_Interval = format(!!sym(interval_col_expected), "%Y-%m-%d %H:%M:%S")) %>%
    distinct(Unique_Interval)
  
  # Extract unique date-time intervals from the actual data DataFrame
  unique_intervals_actual <- actual_data_df %>%
    mutate(Unique_Interval = format(!!sym(interval_col_actual), "%Y-%m-%d %H:%M:%S")) %>%
    distinct(Unique_Interval)
  
  # Find the date-time intervals that are in expected but missing in actual data
  missing_intervals <- setdiff(unique_intervals_expected$Unique_Interval, unique_intervals_actual$Unique_Interval)
...
# Example usage:
find_missing_datetime_intervals(intervals_df, pickup_counts_1000m, "Interval", "time_interval")
...
```{r}
# Remove duplicates in train_filtered_1000m_data
train_filtered_1000m_data <- train_filtered_1000m_data %>%
  distinct(rounded_timestamp, .keep_all = TRUE)
train_filtered_1000m_data
```
