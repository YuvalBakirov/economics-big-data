...
```{r}
# Function to plot Feels Like Temperature vs Uber Demand per week
plot_feels_like_vs_pickups_weekly <- function(data, dataset_name, color) {
  
  # Summarize weekly feels_like and number_of_pickups
  weekly_data <- data %>%
    group_by(week = floor_date(as.Date(time_interval), "week")) %>%
    summarise(
      avg_feels_like = mean(feels_like, na.rm = TRUE),
      total_pickups = sum(number_of_pickups, na.rm = TRUE)
    )
  
  # Create plot with color gradient for feels_like temperature
  ggplot(weekly_data, aes(x = week)) +
    geom_line(aes(y = total_pickups), color = color, size = 1) +  # Pickups line in blue
    geom_point(aes(y = total_pickups, color = avg_feels_like), size = 3) +  # Points colored by feels_like
    scale_color_gradient(low = "blue", high = "red", name = "Feels Like (°C)") +  # Temperature gradient
    labs(
      title = paste("Feels Like Temperature vs Uber Demand (Weekly) -", dataset_name),
      x = "Week",
      y = "Number of Pickups"
    ) +
    theme_minimal() +
    scale_x_date(date_labels = "%b %d", date_breaks = "1 week") +  # Custom x-axis formatting
...
for 1000m:
```{r}
plot_feels_like_vs_pickups_weekly(train_filtered_1000m_data, "1000m Data", "lightblue")
```
