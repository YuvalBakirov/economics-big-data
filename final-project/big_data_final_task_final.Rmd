...
Visualize the distribution of Uber pickups by hour to check if there are any outside 18:00-23:45:
```{r}
...
```
As seen in the two graphs, there are significant amounts of data outside the desired range of 18:00 to 23:45 for both datasets (1000m and 2000m data). Therefore, to focus on the data for pickups that occurred between 18:00 and 23:45, we will apply a filter to exclude the hours outside this range.

Filter data to include only pickups between 18:00 and 23:45:
```{r}
# Function to filter data between specified date-time range and specific hours (18:00 to 23:45)
filter_time_interval <- function(data, timestamp_column, start_datetime, end_datetime) {
  ...
  filtered_data <- data %>%
    filter(
      ...
      (hour(!!sym(timestamp_column)) >= 18 & hour(!!sym(timestamp_column)) <= 23)
    )
...
train_filtered_1000m_data <- filter_time_interval(
  ...
  start_datetime = "2014-04-01 18:00:00",
...
plot_hourly_pickup_distribution(train_filtered_1000m_data, 
                                "Filtered Uber Pickups by Hour (1000m Data, Expected: 18:00 to 23:45)", 
                                "lightblue")
```
```{r}
# Plot for 2000-meter filtered data (lightgreen color)
plot_hourly_pickup_distribution(train_filtered_2000m_data, 
                                "Filtered Uber Pickups by Hour (2000m Data, Expected: 18:00 to 23:45)", 
                                "lightgreen")
```
...
```
Based on the plot for the 1000-meter radius:

* The pickup counts in the top 10 intervals vary between approximately 140 and 160 pickups per interval.
* The busiest intervals are spread across various dates, with a noticeable peak on July 15, 2014. The majority of these intervals are clustered around evening hours, specifically from 18:00 to 19:45.
* This suggests a consistent high demand for Uber rides near the Empire State Building within a 1000-meter radius during these dates and times, particularly in the evening hours.

```{r}
# Plot for 2000-meter filtered data (lightgreen color)
plot_top_10_pickup_intervals(pickup_counts_2000m, "Top 10 Pickup Intervals (2000m Radius)", "lightgreen")
```
Based on the plot for the 2000-meter radius:

* The pickup counts in the top 10 intervals are around 475-500 pickups per interval.
* The busiest intervals are mostly concentrated on September 6, 2014, with pickup times spanning consistently between 19:00 and 23:00. There are also busy intervals on other dates, including August 13 and September 5.
* This suggests a significant spike in Uber demand within the 2000-meter radius around the Empire State Building, particularly during the evening hours on these specific dates.
...
find_missing_time_intervals <- function(data) {
  ...
  start_time <- as.POSIXct("2014-04-01 18:00:00", tz = "UTC")
  ...
  # Filter the time sequence for intervals between 18:00 and 00:00 only
  expected_time_sequence <- expected_time_sequence[
    format(expected_time_sequence, "%H:%M") >= "18:00" & format(expected_time_sequence, "%H:%M") <= "23:45"
  ]
...
3. Split the time interval to: dayInWeek, dayInMonth, month, year, hour, minute
...
Note: In this case, since our data contains times only between 18:00 and 23:45, we will adapt the time_of_day feature to reflect only the relevant periods: Evening (18:00-20:30) and Night (20:31-23:45). There will be no Afternoon or Morning periods as the data doesn't cover these hours.
...
In our case, Evening (18:00 - 20:30) is represented by 0, and Night (20:31 - 23:45) is represented by 1. 
...
add_is_night  <- function(data) {
  data %>%
    mutate(
      is_night = case_when(
        # Evening: 18:00 - 20:30
        hour(time_interval) == 18 ~ 0,                              
        hour(time_interval) > 18 & hour(time_interval) < 20 ~ 0,        
        hour(time_interval) == 20 & minute(time_interval) <= 30 ~ 0, 
        
        # Night: 20:31 - 00:00 (not including)
...
##### split the time intervals
```{r}
# Function to split time_interval into dayInWeek, dayInMonth, month, year, hour, minute
split_time_interval <- function(df) {
  
  # Ensure the time_interval is in POSIXct format
  df$time_interval <- as.POSIXct(df$time_interval, format = "%Y-%m-%d %H:%M:%S")
  
  # Extract numeric components
  df$dayInWeek <- as.numeric(format(df$time_interval, "%u"))  # Day of the week (1 = Monday, 7 = Sunday)
  df$dayInMonth <- as.numeric(format(df$time_interval, "%d")) # Day of the month
  df$month <- as.numeric(format(df$time_interval, "%m"))      # Month
  df$year <- as.numeric(format(df$time_interval, "%Y"))       # Year
  df$hour <- as.numeric(format(df$time_interval, "%H"))       # Hour
  df$minute <- as.numeric(format(df$time_interval, "%M"))       # Hour
...
```{r}
# Apply the function to the 1000m dataset
train_filtered_1000m_data <- split_time_interval(train_filtered_1000m_data)
train_filtered_1000m_data
```

```{r}
# Apply the function to the 2000m dataset
train_filtered_2000m_data <- split_time_interval(train_filtered_2000m_data)
train_filtered_2000m_data
```

```{r}
# Apply the function to the test dataset
test_fixed_data <- split_time_interval(test_fixed_data)
test_fixed_data
```
...
final dfs before changing them in the EDA:
```{r}
train_1000m_data_copy <- train_filtered_1000m_data
train_2000m_data_copy <- train_filtered_2000m_data
test_data_copy <- test_fixed_data
```
...
numerical_features <- c(
  ...
  "number_of_crashes", "event_count", "taxis_pickup_count", "total_arrests", "dayInWeek",
  "dayInMonth", "month", "year", "hour", "minute"
)
...
* temp and feels_like (0.99): This extremely high correlation indicates near redundancy. We’ll likely drop one of these, probably feels_like, to minimize multicollinearity.

2. Correlation with number_of_pickups:
* Positive Correlations:
- taxis_pickup_count (0.4) and total_arrests (0.36): These show moderate positive correlations with Uber pickups, suggesting that higher demand for Uber may be linked to general transportation activity and arrest rates in the area.
Negative Correlations:

* Low Correlations:
- wind_speed (-0.03) and number_of_persons_injured (0.01): These features have minimal impact on Uber demand, making them less valuable as predictors and potential candidates for exclusion.

3. Handling Missing Values (NAs):
* number_of_persons_killed contains missing values (NAs), which we’ll handle in a later preprocessing step.
* year is consistently NA due to having only one year (2014) in the dataset; therefore, it will be removed as it doesn’t add any variability.

4. Key Takeaways:
* Multicollinearity: We’ll reduce redundancy by addressing high correlations, specifically between temp and feels_like.
* Predictive Features: Variables like taxis_pickup_count and total_arrests appear to influence Uber demand.
* Low Impact Features: Variables with low correlations to number_of_pickups, such as wind_speed and number_of_persons_injured, may be removed from the model due to minimal predictive value.

5. Additional Notes:
Low Correlation with Temporal Variables: Variables like hour, minute, and dayInMonth have low correlations with number_of_pickups. We’ll retain them initially but may consider removal in further refinement if they don’t contribute meaningfully to the model.
...
1. High Correlation Between Features (Potential Multicollinearity):
* temp and feels_like (0.99): This nearly perfect correlation suggests redundancy. We will likely drop feels_like to minimize multicollinearity.

2. Correlation with number_of_pickups:
* Positive Correlations:
- taxis_pickup_count (0.38) and total_arrests (0.28): These moderate correlations suggest that higher transportation demand (taxi activity) and arrest rates are associated with an increase in Uber pickups.
- event_count (0.36) and number_of_crashes (0.26): Both events and traffic incidents appear linked to increased Uber demand, likely due to congestion or disrupted transportation options, encouraging people to choose Uber.

* Low Correlations:
- wind_speed (-0.08) and number_of_persons_killed (0.01): These features show very weak correlations with Uber pickups, indicating minimal impact on demand. They are likely not useful predictors for the model.

3. Handling Missing Values (NAs):
year is consistently NA due to having only one year (2014) in the dataset. We’ll remove it, as it does not contribute meaningful variability.

4. Key Takeaways:
* Multicollinearity: We’ll focus on reducing redundancy by addressing the high correlation between temp and feels_like.
Predictive Features: Variables like taxis_pickup_count, total_arrests, event_count, and number_of_crashes show notable relationships with Uber demand, making them valuable predictors in the model.
* Low Impact Features: Features with minimal correlation, such as wind_speed and number_of_persons_killed, may not significantly contribute to the model and could be removed to streamline analysis.

5. Additional Notes:
Low Correlation with Temporal Variables: Variables such as minute and dayInMonth have low correlations with number_of_pickups. We’ll retain them initially but may consider removal if they prove to be uninformative in later analysis.
...
Also, this is the same explanation for year feature (we only habe one year of 2014).
...
In addition, the correlation with the label "number_of_pickups" we calculated earlier for the 2000 table was also very negligible (0.01).
...
```{r}
features_to_remove <- c("is_snowing_last_hour", "number_of_persons_killed", "year")
```
...
calculate_vif <- function(data, dataset_name, numerical_features) {
  numerical_features<- setdiff(numerical_features, c("is_snowing_last_hour", "number_of_persons_killed", "year"))
...
```{r}
features_to_remove <- append(features_to_remove, "temp")
```
...
1. wind_speed:
* The correlation between wind_speed and number_of_pickups is very low.
...
2. clouds_all:
* The correlation between clouds_all and number_of_pickups is weak.
...
3. humidity:
* The correlation between humidity and number_of_pickups is weak.
...
```{r}
features_to_remove <- append(features_to_remove, c("wind_speed", "clouds_all", "humidity"))
```
...
1. Uber Pickup Counts:
The number of pickups ranges from 0 to a maximum of 190, with a mean of 44.3 pickups per interval. This suggests a wide variation in demand within the 1000m radius.

2. Time Intervals:
Data covers the period from April 1, 2014, to September 9, 2014, with intervals between 18:00 and 23:45.
The distribution across hour and minute is quite broad, supporting detailed time-based analysis.

3. Feels Like Temperature:
The "feels like" temperature ranges from 0.53°C to 37.03°C, with a mean of 22.4°C. This dataset largely represents the warmer months, impacting people's likelihood of venturing out and taking Uber.

4. Rain Events:
is_raining_last_hour is mostly 0 (no rain) with a mean of 0.135, indicating that rain events are relatively infrequent in this dataset.

5. Crash and Injury Data:
number_of_persons_injured and number_of_crashes show low counts, with a maximum of 5 injuries and 3 crashes, indicating that serious incidents are rare but included in the data.

6. External Events and Holidays:
The dataset includes binary indicators for major holidays: memorial_day, independence_day, and labor_day, each occurring infrequently (mean values close to 0). This allows for examining holiday impacts on Uber demand.

7. Other Features:
event_count, taxis_pickup_count, and total_arrests provide additional context, with a moderate mean number of events and taxi pickups (43.2 and 667.6, respectively) per interval. These features likely capture the general demand level in the area.

In summary, this dataset captures a detailed view of Uber demand in a small radius around the Empire State Building, with additional weather, event, and holiday variables that can help analyze the drivers of Uber demand in this busy urban area. The presence of both time-based variables (hour, minute) and contextual indicators (temperature, rain, events) supports a comprehensive analysis of demand patterns.


* Key Observations from the 2000m Dataset:
1. Uber Pickup Counts:
The number of pickups ranges from 0 to a maximum of 519, with a mean of 210.6 pickups per interval. This suggests higher demand overall compared to the 1000m dataset, reflecting a broader catchment area.

2. Time Intervals:
The dataset spans from April 1, 2014, to September 9, 2014, with intervals between 18:00 and 23:45, similar to the 1000m data. This allows for detailed analysis of evening and night-time Uber demand patterns.

3. Feels Like Temperature:
The "feels like" temperature ranges from 0.53°C to 37.03°C, with a mean of 22.4°C, covering the warmer months. This range has a likely impact on people’s decision to use Uber, especially in moderate temperatures.

4. Rain Events:
is_raining_last_hour is mostly 0 (no rain) with a mean of 0.135, indicating infrequent rain events in this data, similar to the 1000m dataset.

5. Crash and Injury Data:
number_of_persons_injured and number_of_crashes show higher counts compared to the 1000m data, with a mean of 1.485 injured persons and 4.669 crashes per interval. The maximum values are also notably higher (up to 20 injuries and 18 crashes), suggesting that more incidents are captured in the larger radius.

6. External Events and Holidays:
The binary indicators for major holidays (memorial_day, independence_day, labor_day) appear infrequently, as expected (mean values close to 0). This provides the opportunity to analyze holiday impacts on demand.

7. Other Features:
event_count, taxis_pickup_count, and total_arrests are higher in this dataset, with total arrests having a mean of 945.9 and a maximum of 1314. This indicates a higher level of activity in this radius, likely capturing various urban events and public activities.

8. Comparison with 1000m Dataset:
Compared to the 1000m radius, the 2000m dataset has significantly higher average and maximum pickup counts, suggesting a wider variety of trip purposes beyond commuting. The larger area includes more diverse locations that likely sustain demand into the later hours, reflecting nightlife, dining, and other activities beyond commuting.

In summary, the 2000m dataset captures a broader view of Uber demand around the Empire State Building. The increased pickup counts, higher event and arrest counts, and greater variation in crash and injury data suggest that this dataset includes a more complex mix of urban factors influencing Uber demand. This range of variables allows for analysis of how these factors collectively impact ride-hailing in a busy metropolitan environment.
...
* There are fewer instances with a high number of pickups (120+), suggesting that higher demand events or locations are less frequent within this smaller radius.
...
3.  2014-09-09 - We will take this day with a limited guarantee. This is the last day in our data when it had missing intervals that we filled with zeros.
...
plot_avg_pickups_by_day_of_week <- function(df, data_name) {
  ...
  time_order <- c('18:00', '18:15', '18:30', '18:45',
...
Key Insights:
- Higher Demand at the Start of Evenings: All days peak around 18:00, indicating this is a popular time for Uber pickups, likely due to people finishing work or evening commutes.
...
```
Based on the updated plots for the 1000-meter radius:

Weekdays: Uber demand is highest at the start of the evening around 18:00, with a consistent decline in pickups as the night progresses. This likely reflects evening commuting patterns, with demand peaking as people leave work and gradually decreasing into the late evening.

Weekends: Demand is lower overall, with a more fluctuating pattern throughout the evening. There are smaller, periodic peaks rather than a single downward trend, indicating that weekend trips are more sporadic and likely associated with social activities and leisure.

In summary, Uber demand on weekdays is structured and declines as the evening goes on, while on weekends, demand is lower and varies throughout the night, reflecting more casual usage.
...
Weekdays: Uber demand remains high and stable across most intervals, with pickups spread consistently from 18:00 onward, reflecting routine commuter activity.
Weekends: Demand generally appears lower than on weekdays and is more varied, with occasional peaks. This scattered pattern aligns with weekend-specific events or social activities.

Hourly Demand Trends:
Weekdays: Pickup counts start high around 18:00 and gradually decrease after 21:00, dropping significantly closer to midnight.
Weekends: The pickup rate also begins high in the early evening but fluctuates more, showing dips and rises throughout the night. This irregular pattern likely reflects social outings and varied weekend schedules.

Summary: The 2000m data reveals that weekday evenings have more predictable, consistent Uber demand, while weekends display a more varied pattern, driven by social and leisure activities. This distinction highlights the routine nature of weekday commuting compared to the dynamic nature of weekend activities.

---------------------

The comparison between the 1000m and 2000m datasets provides insights into how Uber demand varies around the Empire State Building area:

1000m Radius: This area shows higher peaks in demand, particularly during weekday rush hours, reflecting concentrated commuter and tourist activity in central Manhattan. On weekends, demand is lower, with fluctuations likely tied to more varied tourist and leisure activity.

2000m Radius: Covering a larger area, this dataset shows more stable demand patterns, indicative of the broader mix of residential, business, and event-driven activity in a wider urban zone. Weekday demand here remains steady through the evening, with a smoother decline. Weekend demand, while variable, maintains a steady baseline due to a mix of social and leisure outings.

In summary, Uber demand in the 1000m radius is more affected by the intense traffic around the Empire State, showing sharp demand fluctuations. In contrast, the 2000m radius reflects a more consistent urban demand, encompassing diverse transportation needs beyond the immediate high-traffic zone.
...
create_bin_plot <- function(df, data_name) {
  ...
  # Define the evening and night intervals
  evening_intervals <- c('18:00', '18:15', '18:30', '18:45',
...
The bar charts illustrate Uber demand across evening and night intervals within 1000m and 2000m radii of the Empire State Building.

In the 1000m radius, demand peaks between 18:00 and 19:30, aligning with the end of the workday, and then gradually declines throughout the evening and night. This suggests that Uber is heavily used for post-work and early evening activities within the close vicinity of the Empire State Building.

In the 2000m radius, while the evening peak pattern is similar, the total pickup count is significantly higher, and the demand remains strong across more intervals into the night. This broader area likely captures diverse activities and nightlife, sustaining demand later into the evening.

Overall, the 2000m radius demonstrates higher and more sustained demand compared to the 1000m radius, reflecting its inclusion of a wider mix of residential, business, and entertainment venues.
...
for 1000m:
```{r}
plot_uber_demand_for_time_interval(train_filtered_1000m_data, "1000m Data", "18:00", "lightblue")
```
...
We saw earlier that the peak hours for both of that dfs: Data 1000 & Data 2000 is 18:00 (hour with the biggest amount of uber pickups overall).
During rush hour (18:00 for 1000m & 2000m dfs), people tend to take Uber regardless of whether it is raining or not. This is likely due to the necessity of commuting at those times. However, we see similar trends in pickups, where rain does not seem to significantly alter the demand pattern. This is expected during rush hours when people prioritize getting home or to work, rain or shine.
...
The "feels like" temperature has a clear impact on Uber demand around the Empire State Building.

1. Moderate Temperatures Drive Demand: Most Uber pickups occur when the "feels like" temperature is moderately warm (18.8 - 27.9°C). In both the 1000m and 2000m datasets, over half of the pickups happen in this range, indicating that people are more likely to be out and use Uber services when temperatures are comfortable.

2. Extreme Temperatures Decrease Demand: During high temperatures (27.9 - 37°C) and low temperatures (0.5 - 9.7°C), the demand for Uber decreases. This suggests that people are less inclined to venture out when it feels very hot or relatively cold, possibly opting to stay indoors or work from home to avoid discomfort.

3. Balanced Time Distribution Reference: The time distribution bar beside each temperature range in the legend shows that these insights are not solely due to time spent in each range. Even with nearly 55.6% of time intervals in the mid-low range, the highest demand is still in the mid-high range, reinforcing the "feels like" temperature’s influence on Uber demand.

In summary, people prefer using Uber during comfortable temperatures, while extreme weather conditions—both hot and cold—lead to a noticeable drop in demand.
