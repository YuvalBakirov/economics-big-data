...
We saw before that for both of that data-sets the week starting at June 30, 2014 between a drop in demand for Uber, we'll take a closer look at it:
```{r}
...
```{r}
plot_histogram(train_filtered_1000m_data, "1000m Data", "number_of_persons_injured", "lightgreen")
```
...
plot_avg_pickups_per_crash <- function(df, data_name) {
...
```{r}
plot_avg_pickups_per_crash(train_filtered_2000m_data, "2000m Data")
```
...
```{r}
# Function to create Event Clustering Visualization with a trend line
event_clustering_visualization <- function(data, dataset_name, color_low, color_mid, color_high) {
  ...
  # Create a scatter plot to visualize clusters with a trend line
...
The total number of events in New York City includes a broad range of activities, from large gatherings to smaller local events. When analyzing Uber demand within a 1000-meter radius of the Empire State Building, the impact of these events appears minimal. This is likely because the event data represents activities across the entire city, many of which may not affect the immediate area around the Empire State. As a result, the trend line shows a slightly negative slope (almost a straight line), suggesting no significant correlation between the number of events and Uber demand. The small negative trend may also be due to outliers at the far end of the total events axis.

However, in a larger 2000-meter radius, covering more parts of New York and extending into Newark, the relationship between total events and Uber demand becomes clearer. The trend line here shows a positive slope, indicating that as the number of total events increases, so does the minimum demand for Uber, suggesting that events spread across a broader area begin to have a greater influence on transportation needs.

Given the minimal impact of total events on Uber demand in the smaller 1000-meter radius and the slightly negative trend, we will remove the total_events feature from the 1000-meter dataset to focus on more relevant factors for predicting demand.
...
In these visualizations, we observe how the different combinations of "feels like" temperature and humidity impact Uber demand. For both the 1000-meter and 2000-meter radius data, the largest percentage of Uber pickups occurs in conditions where the "feels like" temperature is high and the humidity is low, followed by the group with high "feels like" and high humidity.

The relatively smaller percentages of Uber demand in the groups where "feels like" is low, regardless of humidity, suggest that the "feels like" temperature plays a more significant role in influencing demand compared to humidity. While humidity may have some impact, especially in conjunction with high "feels like" temperatures, its overall influence appears limited.

Given this distribution, it might be more efficient to focus on "feels like" temperature as the primary factor and remove humidity from the analysis. This would streamline the features being used in modeling without sacrificing much predictive power, as the impact of humidity alone seems marginal compared to the stronger effect of "feels like" temperature on Uber demand.

```{r}
# Function to remove 'humidity' feature
remove_humidity_feature <- function(df) {
  
  # Remove 'humidity' column from the dataframe if it exists
  if ("humidity" %in% colnames(df)) {
    df <- df %>% select(-humidity)
  }
...
```{r}
# Update with is_holiday feature
train_filtered_1000m_data <- create_is_holiday_feature(train_filtered_1000m_data)

# Update with is_persons_injured feature
train_filtered_1000m_data <- create_is_persons_injured_feature(train_filtered_1000m_data)

# Remove total_events
train_filtered_1000m_data <- remove_total_events_1000m(train_filtered_1000m_data)

# Remove humidity
train_filtered_1000m_data <- remove_humidity_feature(train_filtered_1000m_data)
```
...
num_remove <- c("humidity", "event_count", "number_of_persons_injured")
...
```{r}
# Update with is_holiday feature
train_filtered_2000m_data <- create_is_holiday_feature(train_filtered_2000m_data)

# Update with is_persons_injured feature
train_filtered_2000m_data <- create_is_persons_injured_feature(train_filtered_2000m_data)

# Remove humidity
train_filtered_2000m_data <- remove_humidity_feature(train_filtered_2000m_data)
```
...
num_remove <- c("humidity", "number_of_persons_injured")
numerical_features_2000 <- setdiff(numerical_features_2000, num_remove)
