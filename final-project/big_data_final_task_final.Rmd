...
TODO - גרף סהכ יומי ושבועי, לראות אם יש ימים חריגים שלא אמורים להיות כל שנה
לראות בסהכ השבועי שיכול להסביר את נפילה ואת העליה באוגוסט
לראות אם החגים מתנהגים דומה אם כן נעשה פיצאר אחד לחגים

סהכ יומי וסהכ שעות עבודה בגרף - לראות  אם יש התנהגות שונה
...
הגרף מציג את הביקוש לנסיעות אובר בתוך רדיוס של 1000 מטר ממגדל האמפייר סטייט. הקו הכחול מייצג את הנתונים המקוריים, והקו הירוק, הכתום והאדום מייצגים את הממוצעים הנעים היומיים, השבועיים והחודשיים בהתאמה. רואים תנודות משמעותיות בביקוש לאורך כל התקופה, כאשר השיאים קורים בזמנים מסוימים של היום או עקב אירועים מקומיים. המגמה החודשית (אדום) מראה עלייה מתונה בביקוש עד יולי, ואחריה ירידה קלה באוגוסט וספטמבר (למרות שחסרים נתונים לחלק מספטמבר).

בגרף השני (2000m ומעלה):

הגרף מציג את הביקוש לנסיעות אובר מעבר לרדיוס של 2000 מטר ממגדל האמפייר סטייט. כמו בגרף הראשון, גם כאן יש תנודות משמעותיות בנתונים המקוריים (כחול), עם מגמות יומיות, שבועיות וחודשיות (ירוק, כתום, אדום). הביקוש כאן גבוה יותר ומכיל הרבה יותר תנודות לעומת הגרף הראשון, מה שמעיד על שטח רחב יותר שכולל יותר אזורים תיירותיים ואזורי עסקים. כמו בגרף הקודם, יש עלייה מתונה בביקוש עד יולי, ולאחר מכן ירידה מסוימת.
...
Top 3 days with the lowest and highest total number of pickups:
```{r}
# Function to show top n days with lowest and highest total number of pickups
plot_top_lowest_highest_pickups <- function(data, feature, color, dataset_name, n) {
...
```{r}
plots <- plot_top_lowest_highest_pickups(train_filtered_1000m_data, "number_of_pickups", "lightblue", "1000m Data", 5)
plots
```
...
* Daily Picks:
(If these events were one-time occurrences, they might be considered outliers in the data, as they caused an unusual spike in transportation demand on those particular days.)
- On May 1, 2014, the Empire State Building celebrated its 83rd anniversary. the Empire State Building often hosts celebrations involving special lighting, VIP tours, or other commemorative activities around May 1, the anniversary date. This is probably the reason for the high demand for Uber on April 30.
- On July 15, 2014, one major event in New York was Bruno Mars' concert at Madison Square Garden, which was part of his "Moonshine Jungle Tour." This event likely contributed to the spike in Uber pickups, as concerts at such a large venue can draw significant crowds, increasing the need for transportation services. 
- Regarding September 5, 2014, this date coincides with the US Open Tennis Championships, which take place annually in New York at Flushing Meadows. This major international sporting event attracts tens of thousands of spectators, which would have increased transportation demand, explaining the higher Uber pickup numbers.
...
```{r}
plots <- plot_top_lowest_highest_pickups(train_filtered_2000m_data, "number_of_pickups", "lightgreen", "2000m Data", 5)
plots
```
...
```{r}
# Call the function for 1000m data
plot_weekly_behavior(train_filtered_1000m_data, "taxis_pickup_count", "1000m Data", "lightblue")
```

day:
```{r}
# Call the function for 1000m data
plot_daily_behavior(train_filtered_1000m_data, "taxis_pickup_count", "1000m Data", "lightblue")
```

```{r}
plots <- plot_top_lowest_highest_pickups(train_filtered_1000m_data, "taxis_pickup_count", "lightblue", "1000m Data", 5)
plots
```
...
```{r}
# Call the function for 1000m data
plot_weekly_behavior(train_filtered_2000m_data, "taxis_pickup_count", "2000m Data", "lightgreen")
```

day:
```{r}
# Call the function for 1000m data
plot_daily_behavior(train_filtered_2000m_data, "taxis_pickup_count", "2000m Data", "lightgreen")
```

```{r}
plots <- plot_top_lowest_highest_pickups(train_filtered_2000m_data, "taxis_pickup_count", "lightgreen", "2000m Data", 5)
plots
```
...
```{r}
# Function to plot daily, weekly, and monthly behavior with dual y-axes
plot_behavior_dual_axis <- function(data, feature, dataset_name, color_pickups, color_feature) {
  ...
  # Function to create the dual-axis plot
  plot_dual <- function(data, x_col, x_label, title) {
    ggplot(data, aes(x = !!sym(x_col), group = 1)) +  # Adding group = 1 for proper line connection
      ...
      geom_line(aes(y = total_feature, color = "Total Feature"), size = 1, linetype = "dashed") +
      geom_point(aes(y = total_feature, color = "Total Feature"), size = 2) +
      scale_y_continuous(
        name = "Total Number of Pickups",
        sec.axis = sec_axis(~., name = paste("Total", feature))  # Secondary axis for feature
      ) +
      labs(
        title = title,
        x = x_label
      ...
      scale_color_manual(name = "Trends", 
                         values = c("Total Feature" = color_feature, "Total Pickups" = color_pickups)) +
      theme_minimal() +
      theme(axis.text.x = element_text(angle = 45, hjust = 1),
...
for 1000m:
```{r}
plots <- plot_behavior_dual_axis(train_filtered_1000m_data, "taxis_pickup_count", "1000m Data", "lightblue", "blue")
plots
```
...
for 2000m:
```{r}
plots <- plot_behavior_dual_axis(train_filtered_2000m_data, "taxis_pickup_count", "2000m Data", "lightgreen", "green")
plots
```
...
We will say that a day is rainy if it had more than two hours of rain, that is, a day in which there are 8 intervals of 15 minutes with is_raining_last_hour = 1 is considered a rainy day. 
We would like to check on a graph how rainy days affect the demand for rain
```{r}
#todo
```
...
for 1000:
```{r}
plots <- plot_behavior_dual_axis(train_filtered_1000m_data, "total_arrests", "1000m Data", "lightblue", "blue")
plots
```
...
for 2000:
```{r}
plots <- plot_behavior_dual_axis(train_filtered_2000m_data, "total_arrests", "2000m Data", "lightgreen", "green")
plots
```
...
```{r}
# Function to create Event Clustering Visualization
event_clustering_visualization <- function(data, dataset_name, color_low, color_mid, color_high) {
  ...
  ggplot(data, aes(x = total_events, y = total_pickups, color = event_cluster)) +
    geom_point(size = 3) +
    scale_color_manual(values = c("Low" = color_low, "Moderate" = color_mid, "High" = color_high)) +
    labs(
      title = paste("Event Clustering vs Uber Demand -", dataset_name),
      x = "Total Events",
      y = "Total Uber Pickups",
      color = "Event Clusters"
    ) +
    theme_minimal() +
...
for 1000m:
```{r}
event_clustering_visualization(train_filtered_1000m_data, "1000m Data", "blue", "yellow", "red")
```

for 2000m:
```{r}
event_clustering_visualization(train_filtered_2000m_data, "2000m Data", "blue", "yellow", "red")
```
...
```{r}
# Function to define thresholds, print them, and plot the behavior with percentages on top of bars
plot_feels_like_humidity_vs_pickups <- function(data, dataset_name) {
  # Calculate range for humidity and feels_like
  humidity_range <- max(data$humidity, na.rm = TRUE) - min(data$humidity, na.rm = TRUE)
  feels_like_range <- max(data$feels_like, na.rm = TRUE) - min(data$feels_like, na.rm = TRUE)
  
  # Define thresholds for high and low humidity and feels_like
  low_humidity_threshold <- min(data$humidity, na.rm = TRUE)
  high_humidity_threshold <- low_humidity_threshold + humidity_range / 2
  low_feels_like_threshold <- min(data$feels_like, na.rm = TRUE)
  high_feels_like_threshold <- low_feels_like_threshold + feels_like_range / 2
  
  # Print the thresholds
  cat("Feels Like Range:\n")
  cat("Low: ", low_feels_like_threshold, "-", high_feels_like_threshold, "\n")
  cat("High: ", high_feels_like_threshold, "-", max(data$feels_like, na.rm = TRUE), "\n\n")
  
  cat("Humidity Range:\n")
  cat("Low: ", low_humidity_threshold, "-", high_humidity_threshold, "\n")
  cat("High: ", high_humidity_threshold, "-", max(data$humidity, na.rm = TRUE), "\n\n")
  
  # Categorize data into four groups based on humidity and feels_like
  data <- data %>%
    mutate(
      humidity_group = ifelse(humidity < high_humidity_threshold, "Low Humidity", "High Humidity"),
      feels_like_group = ifelse(feels_like < high_feels_like_threshold, "Low Feels Like", "High Feels Like"),
      group = paste(humidity_group, feels_like_group, sep = " - ")
    )
  ...
  # Create a bar plot with percentages on top of the bars
  ggplot(group_summary, aes(x = group, y = pickup_percentage, fill = group)) +
    geom_bar(stat = "identity", color = "black") +
    geom_text(aes(label = sprintf("%.1f%%", pickup_percentage)), vjust = -0.5, size = 5) +  # Add percentages on top of bars
    labs(
      title = paste("Uber Demand by Feels Like and Humidity Groups -", dataset_name),
      x = "Group",
      y = "Percentage of Total Uber Pickups (%)"
    ) +
    scale_fill_manual(values = c("High Humidity - High Feels Like" = "red", 
                                 "High Humidity - Low Feels Like" = "orange",
                                 "Low Humidity - High Feels Like" = "purple",
                                 "Low Humidity - Low Feels Like" = "blue")) +
...
```{r}
plot_feels_like_humidity_vs_pickups(train_filtered_1000m_data, "1000m Data")
```
```{r}
plot_feels_like_humidity_vs_pickups(train_filtered_2000m_data, "2000m Data")
```

From the visualizations, we can draw some insights regarding the relationship between Uber demand, "feels like" temperature, and humidity levels:
* High Uber Demand: The group "Low Humidity - High Feels Like" consistently exhibits the highest Uber demand in both 1000m and 2000m datasets, representing 45.4% and 47.2% of total pickups, respectively. This could suggest that warmer temperatures with lower humidity might encourage people to use Uber services more frequently.
* Lower Demand in High Humidity: The "High Humidity - Low Feels Like" group has the lowest Uber demand in both cases, suggesting that cooler, humid weather might be less conducive to Uber usage.
* General Influence of Humidity and Feels Like: The division of Uber demand between high and low humidity and feels-like temperatures demonstrates the nuanced relationship between weather conditions and transportation preferences.

```{r}
# Function to plot daily behavior with humidity and feels_like groups
plot_feels_like_humidity_grouped <- function(data, dataset_name, color) {
  # Calculate range for humidity and feels_like
  humidity_range <- max(data$humidity, na.rm = TRUE) - min(data$humidity, na.rm = TRUE)
  feels_like_range <- max(data$feels_like, na.rm = TRUE) - min(data$feels_like, na.rm = TRUE)
  ...
  daily_data <- data %>%
    group_by(day = as.Date(time_interval), group) %>%
    summarise(total_pickups = sum(number_of_pickups, na.rm = TRUE), .groups = "drop")  # Suppressing the warning here
  
  # Create a line plot with colored points based on the groupings
  ggplot(daily_data, aes(x = day, y = total_pickups)) +
    geom_line(color = color, size = 1) +  # Line for total pickups
    geom_point(aes(color = group), size = 1.5) +  # Color points based on group
    scale_color_manual(
      values = c("High Humidity - High Feels Like" = "red", 
                 "High Humidity - Low Feels Like" = "orange",
                 "Low Humidity - High Feels Like" = "purple",
                 "Low Humidity - Low Feels Like" = "blue"),
      name = "Humidity and Feels Like Groups\n"
    ) +
    labs(
      title = paste("Daily Uber Demand by Feels Like and Humidity Groups -", dataset_name),
      x = "Day",
      y = "Total Pickups"
    ) +
    theme_minimal() +
...
```{r}
plot_feels_like_humidity_grouped(train_filtered_1000m_data, "1000m Data", "lightblue")
```

```{r}
plot_feels_like_humidity_grouped(train_filtered_2000m_data, "2000m Data", "lightgreen")
```


From the visualizations, it’s possible to observe some trends:
* High Humidity - High Feels Like (red points) are more prevalent during summer months, especially in July and August, which could correspond to high demand for Uber during hot, sticky weather, especially on days with more pickups.
* Low Humidity - Low Feels Like (blue points) tends to dominate during cooler months, such as April and early May, where demand seems lower.
* Seasonal Patterns: The general demand increases toward the summer months, but the effect of temperature and humidity variations can be more clearly seen during peak months.

The combination of High Humidity - High Feels Like may contribute to higher demand, while Low Humidity - Low Feels Like appears on days with fewer pickups. This shows a potential correlation between weather conditions and Uber demand, suggesting that people may avoid walking in uncomfortable weather, increasing Uber usage.
...
After analyzing the EDA, we observed that some days have extreme Uber demand due to one-off events. We will treat these as outliers and address them using noise-handling techniques, as is commonly done with time series data. These methods may include applying smoothing techniques, capping extreme values, or employing anomaly detection to reduce the impact of such events on the overall model. This approach will allow us to account for anomalies while preserving the overall trends in the data.
