...
Let's take a look at the taxis' pickup behavior as well:
```{r}
plot_monthly_behavior(train_filtered_1000m_data, "taxis_pickup_count", "1000m Data", "lightblue")
```

```{r}
plot_monthly_behavior(train_filtered_2000m_data, "taxis_pickup_count", "2000m Data", "lightgreen")
```
Like Uber, the demand for yellow taxis also decreased slightly in June (although the drop was not as significant as for Uber, there was still a small decline). We speculate that this happened because June is considered to have the most pleasant weather in New York-it's not too hot for walking and not too cold or rainy to necessitate taking an Uber or taxi. Later, we will see that June falls in the middle in terms of humidity, feels-like temperature, and the number of rainy days compared to other months (though our view of September is limited, as we only have data for the first nine days of the month).

Additionally, June is often a transitional period between the spring tourist season and the summer peak, which might result in a slight dip in overall transportation demand as tourism slows down before ramping up again in July and August. This could explain why both Uber and yellow taxi demand softened during this month.

However, the drop in yellow taxi demand was smaller than Uber's because taxis were still more popular than Uber in 2014, especially in New York City. At the time, yellow taxis dominated the on-demand transportation market, with their well-established presence and infrastructure, while Uber was still gaining ground.

------------

Plot for the number of rainy days per month:
```{r}
# Function to plot the number of rainy days per month
plot_rainy_days_by_month <- function(data, color, data_name) {
  
  # Add a 'date' column to the data for grouping by day
  data <- data %>%
    mutate(date = as.Date(time_interval))  # Extract the date from the time_interval
  
  # Group by date and determine if it rained on that day (at least one interval had rain)
  rainy_days <- data %>%
    group_by(date) %>%
    summarise(rained = max(is_raining_last_hour)) %>%  # Check if it rained in any time interval
    filter(rained == 1)  # Filter to keep only rainy days
  ...
  rainy_days_per_month <- rainy_days %>%
    group_by(month) %>%
    summarise(number_of_rainy_days = n())
  
  # Create the plot
  ggplot(rainy_days_per_month, aes(x = month, y = number_of_rainy_days)) +
    geom_bar(stat = "identity", fill = color, color = "black") +
    labs(
      title = paste("Number of Rainy Days per Month -", data_name),
      x = "Month",
...
```{r}
plot_rainy_days_by_month(train_filtered_1000m_data, "lightblue", "1000m Data")
```
```{r}
plot_rainy_days_by_month(train_filtered_2000m_data, "lightgreen", "2000m Data")
```

------------

Here's a function that plots the mean feature of feels_like & humidity for each month. This temperature accounts for the human perception of weather:
```{r}
# Function to plot the mean weather feature for each month
plot_mean_feature_by_month <- function(data, feature, color, data_name) {
  
  # Add a 'month' column to the data for grouping by month
  data <- data %>%
    mutate(month = floor_date(time_interval, "month"))
  
  # Calculate the mean of the specific feature per month
  feature_means <- data %>%
    group_by(month) %>%
    summarise(
      mean_feature = mean(.data[[feature]], na.rm = TRUE)  # Use the feature argument
    )
  
  # Create a plot to visualize the mean feature per month
  ggplot(feature_means, aes(x = as.factor(month), y = mean_feature)) +
    geom_bar(stat = "identity", fill = color, color = "black") +
    labs(
      title = paste("Mean", feature, "Per Month -", data_name),
      x = "Month",
      y = paste("Mean", feature)
    ) +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
}
...
```{r}
plot_mean_feature_by_month(train_filtered_1000m_data, "feels_like", "lightblue", "1000m Data")
```

```{r}
plot_mean_feature_by_month(train_filtered_2000m_data, "feels_like", "lightgreen", "2000m Data")
```

```{r}
plot_mean_feature_by_month(train_filtered_1000m_data, "humidity", "lightblue", "1000m Data")
```

```{r}
plot_mean_feature_by_month(train_filtered_2000m_data, "humidity", "lightgreen", "2000m Data")
```

------------

Holidays count in every month:
```{r}
# Function to count holidays per month and display as a box plot
count_holidays_by_month <- function(data, color, data_name) {
  
  # Add a 'month' column to the data for grouping by month
  data <- data %>%
    mutate(month = floor_date(time_interval, "month"))
  
  # Count the number of holidays (Memorial Day, Independence Day, Labor Day) per month
  holiday_counts <- data %>%
    group_by(month) %>%
    summarise(
      total_holidays = sum(memorial_day + independence_day + labor_day)
    )
  
  # Create a box plot to visualize holiday counts per month
  ggplot(holiday_counts, aes(x = as.factor(month), y = total_holidays)) +
    geom_boxplot(fill = color, color = color) +
    labs(
      title = paste("Holiday Counts Per Month -", data_name),
      x = "Month",
...
```{r}
count_holidays_by_month(train_filtered_1000m_data, "lightblue", "1000m Data")
```

```{r}
count_holidays_by_month(train_filtered_2000m_data, "lightgreen", "2000m Data")
```

------------

Sum of: event_count, number_of_crashes, total_arrests in every month:
```{r}
# Define the function to create the box plot for any feature's sum by month
plot_feature_sum_by_month <- function(data, feature, color, data_name) {
  
  # Extract the month from the time_interval
  data <- data %>%
    mutate(month = format(as.Date(time_interval), "%Y-%m"))
  
  # Group by month and calculate the sum of the given feature for each month
  monthly_feature_sum <- data %>%
    group_by(month) %>%
    summarise(feature_sum = sum(.data[[feature]], na.rm = TRUE))
  
  # Create the box plot
  ggplot(monthly_feature_sum, aes(x = month, y = feature_sum)) +
    geom_boxplot(fill = color, color = color) +
    scale_y_continuous(labels = comma) +  # Format the y-axis to avoid scientific notation
    labs(
      title = paste("Sum of", feature, "by Month for", data_name),
      x = "Month",
      y = paste("Sum of", feature)
    ) +
    theme_minimal() +
...
```{r}
plot_feature_sum_by_month(train_filtered_1000m_data, "event_count", "lightblue", "1000m Data")
```

```{r}
plot_feature_sum_by_month(train_filtered_2000m_data, "event_count", "lightgreen", "2000m Data")
```

```{r}
plot_feature_sum_by_month(train_filtered_1000m_data, "number_of_crashes", "lightblue", "1000m Data")
```

```{r}
plot_feature_sum_by_month(train_filtered_2000m_data, "number_of_crashes", "lightgreen", "2000m Data")
```

```{r}
plot_feature_sum_by_month(train_filtered_1000m_data, "total_arrests", "lightblue", "1000m Data")
```

```{r}
plot_feature_sum_by_month(train_filtered_2000m_data, "total_arrests", "lightgreen", "2000m Data")
```

------------
###### How Does Rain Affect Uber Demand?

Let calculate the number of intervals where it is raining and where it is not:
```{r}
# Function to calculate the amount of intervals with a specific feature condition
calculate_intervals <- function(data, feature) {
  
  # Count the number of intervals where the feature is true and where it is false
  intervals_summary <- data %>%
    summarise(
      intervals_with_condition = sum(!!sym(feature) == 1, na.rm = TRUE),
      intervals_without_condition = sum(!!sym(feature) == 0, na.rm = TRUE)
    )
...
```{r}
calculate_intervals(train_filtered_1000m_data, "is_raining_last_hour")
```

```{r}
calculate_intervals(train_filtered_2000m_data, "is_raining_last_hour")
```

We can see that our data is not balanced. There are significantly more time intervals without rain than with rain, which makes sense since most of the months we are looking at are during the spring and summer seasons.

Since our data is imbalanced, with many more time intervals without rain than with rain, directly comparing the raw counts of Uber pickups would not provide a fair analysis of how rain affects demand. By using percentages, we can normalize the data and accurately assess the proportionate effect of rain on Uber demand. This approach ensures that we account for the disparity in the number of rainy and non-rainy intervals and provides a clearer comparison. 
```{r}
# Function to compare Uber demand during intervals with or without a specific feature using percentages
plot_uber_demand_by_feature <- function(data, feature, dataset_name, color) {
  
  # Group data by the given feature condition and calculate the total pickups and number of intervals
  ...
  # Calculate the percentage of total pickups for intervals with and without the feature
  feature_summary <- feature_summary %>%
    mutate(pickup_percentage = (total_pickups / sum(total_pickups)) * 100)
  
  # Label for feature condition
  feature_summary[[feature]] <- factor(feature_summary[[feature]], 
                                       levels = c(0, 1), 
                                       labels = c(paste("No", feature), feature))
  
  # Create a bar plot to show percentage of Uber pickups during intervals with and without the feature
  ggplot(feature_summary, aes(x = !!sym(feature), y = pickup_percentage, fill = !!sym(feature))) +
    geom_bar(stat = "identity", color = "black", fill = color) +
    labs(
      title = paste("Uber Demand by", feature, "-", dataset_name),
      x = paste(feature, "Condition"),
      y = "Percentage of Total Uber Pickups (%)"
    ) +
    theme_minimal() +
    scale_y_continuous(limits = c(0, 100), labels = scales::percent_format(scale = 1)) +  # Ensures y-axis always ranges from 0 to 100%
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
}
```

```{r}
plot_uber_demand_by_feature(train_filtered_1000m_data, "is_raining_last_hour", "1000m Data", "lightblue")
```
```{r}
plot_uber_demand_by_feature(train_filtered_1000m_data, "is_raining_last_hour", "2000m Data", "lightgreen")
```

From the charts, we can observe that Uber demand is higher during non-rainy intervals compared to rainy intervals.

In both the 1000m and 2000m data:
* The percentage of Uber pickups during non-rainy periods overwhelmingly dominates, making up more than 75% of the total demand.
* Meanwhile, the percentage of Uber pickups during rainy periods is much lower, accounting for less than 25% of total pickups.

This may suggest that rain, while having an effect on demand, does not substantially increase the number of Uber pickups. This could be because most of the months we're analyzing fall within the spring and summer seasons, where rainy intervals are less frequent. Additionally, in pleasant weather, people may prefer alternative modes of transportation, such as walking, rather than using Uber. However, when it does rain, there is still some observable increase in demand, but it is not as pronounced as one might expect.

------------

###### How Does Weekends Affect Uber Demand?

```{r}
calculate_intervals(train_filtered_1000m_data, "is_weekend")
```

```{r}
calculate_intervals(train_filtered_2000m_data, "is_weekend")
```

We can see that our data is not balanced. There are more time intervals that are not weekends, which makes sense since most of the days in the week do not represent a weekend.

Also here we will look at the %:
```{r}
plot_uber_demand_by_feature(train_filtered_1000m_data, "is_weekend", "1000m Data", "lightblue")
```

```{r}
plot_uber_demand_by_feature(train_filtered_1000m_data, "is_weekend", "2000m Data", "lightgreen")
```

The charts show that the demand for Uber is significantly higher on weekdays compared to weekends. This makes sense because during the week, people are commuting to work, attending meetings, running errands, and generally have more reasons to move around the city. As a result, Uber becomes a convenient mode of transportation to navigate New York's busy streets.

On the other hand, weekends generally have a lower demand for Uber as people might be more likely to relax, stay at home, or use alternative transportation options, especially for leisure activities.

------------

###### How Does the time of the day Affect Uber Demand?
```{r}
calculate_intervals(train_filtered_1000m_data, "is_night")
```

```{r}
calculate_intervals(train_filtered_2000m_data, "is_night")
```

In terms of time of day, the data is quite balanced: that is, there is almost the same amount of night and evening intervals. But to be more precise, we would still prefer to look at the %:
```{r}
plot_uber_demand_by_feature(train_filtered_1000m_data, "is_night", "1000m Data", "lightblue")
```

```{r}
plot_uber_demand_by_feature(train_filtered_1000m_data, "is_night", "2000m Data", "lightgreen")
```


From the charts, we can observe the following insights regarding how the time of day affects Uber demand:

* Demand is significantly higher during the day (non-night intervals): In both the 1000m and 2000m datasets, more than 50% of Uber pickups occur during the day. This makes sense because people are typically commuting, running errands, or engaging in daytime activities during these hours. Daytime tends to be busier, especially in a city like New York, where daytime demand for transportation is generally high.

* Lower demand at night: Nighttime Uber demand is noticeably lower, accounting for less than 50% of total pickups. While there is still demand during night hours (due to activities like dining out, nightlife, or late commutes), the overall volume is lower compared to the daytime.

This suggests that Uber is predominantly used during the day, likely due to regular work-related commutes or daytime activities, while nighttime Uber usage caters to a smaller, more specific set of needs like going out for social events.
...
```{r}
plot_dual_axis <- function(df, pickup_color, crash_color, window = 4) {
  # Calculate rolling means
  df$rolling_pickups <- zoo::rollmean(df$number_of_pickups, window, fill = NA)
  df$rolling_crashes <- zoo::rollmean(df$number_of_crashes, window, fill = NA)
  
  # Remove NA values for plotting
  df_clean <- na.omit(df)
  
  # Create scaling factor to match the ranges of pickups and crashes for visual comparison
  scaleFactor <- max(df_clean$rolling_pickups, na.rm = TRUE) / max(df_clean$rolling_crashes, na.rm = TRUE)

  # Plot with two y-axes
  ggplot(df_clean, aes(x = time_interval)) +
    # Left y-axis for pickups
    geom_line(aes(y = rolling_pickups), size = 1, color = pickup_color) +
    # Right y-axis for crashes (scaled)
    geom_line(aes(y = rolling_crashes * scaleFactor), size = 1, color = crash_color) +
    
    # Labels and titles
    labs(title = paste("Rolling Mean of Pickups and Crashes (Window =", window, ")"),
         x = "Time Interval", y = "Rolling Mean of Pickups") +
    
    # Right y-axis scale for crashes
    scale_y_continuous(sec.axis = sec_axis(~./scaleFactor, name = "Rolling Mean of Crashes")) +
    
    # Annotations for color representation
    annotate("text", x = min(df_clean$time_interval), y = max(df_clean$rolling_pickups, na.rm = TRUE), 
             label = "Pickups", color = pickup_color, size = 5, hjust = 0) +
    annotate("text", x = min(df_clean$time_interval), y = max(df_clean$rolling_pickups, na.rm = TRUE) * 0.95, 
             label = "Crashes", color = crash_color, size = 5, hjust = 0) +
...
```{r}
plot_dual_axis(train_filtered_1000m_data, "#2bcff3", "#4f9ff3", window = 28)
```
Why Window = 28?
* Each day from 17:00 to 00:00 has 7 hours.
* With 4 intervals per hour (since each interval is 15 minutes), the total number of intervals per day is:
7 hours × 4 intervals per hour = 28 intervals per day

Thus, a window of 28 means we are calculating a rolling mean over an entire day (from 17:00 to 00:00). This window size smooths out fluctuations within each day, helping to reveal daily trends in the data.

```{r}
plot_dual_axis(train_filtered_1000m_data, "#2bcff3", "#4f9ff3", window = 196)
```
Calculation for a 7-Day Window:
28 intervals per day × 7 days per week = 196 intervals
a window of 196 would calculate the rolling mean across an entire week.
...
Outliers:
Both when it rains and doesnâ€™t rain, there are outliers (higher pickup numbers). However, there are more extreme values (pickups above 150) when it rains, indicating that rain can sometimes cause a significant increase in demand.
