...
plot_hourly_pickup_distribution(train_filtered_1000m_data, 
                                "Filtered Uber Pickups by Hour (1000m Data, Expected: 18:00 to 00:00)", 
                                "lightblue")
```
```{r}
# Plot for 2000-meter filtered data (lightgreen color)
plot_hourly_pickup_distribution(train_filtered_2000m_data, 
                                "Filtered Uber Pickups by Hour (2000m Data, Expected: 18:00 to 00:00)", 
                                "lightgreen")
```
...
* clouds_all and humidity (0.61): Moderate correlation shows some overlap, but it may not be essential to address unless further analysis suggests otherwise.

2. Correlation with number_of_pickups:
* Positive Correlations:
- taxis_pickup_count (0.31) and total_arrests (0.30): These show moderate positive correlations with Uber pickups, suggesting that higher demand for Uber may be linked to general transportation activity and arrest rates in the area.
Negative Correlations:
- is_night (-0.50) and is_weekend (-0.35): Pickup demand tends to drop significantly at night and during weekends, indicating that these times are associated with lower Uber activity.

* Low Correlations:
- wind_speed (-0.02) and number_of_persons_injured (0.03): These features have minimal impact on Uber demand, making them less valuable as predictors and potential candidates for exclusion.
...
* Predictive Features: Variables like taxis_pickup_count and total_arrests appear to influence Uber demand, while night and weekend indicators are negatively associated with pickups.
...
* clouds_all and humidity (0.61): Moderate correlation shows some overlap, but it is less critical and may not require immediate adjustment.

2. Correlation with number_of_pickups:
* Positive Correlations:
- taxis_pickup_count (0.47) and total_arrests (0.17): These moderate correlations suggest that higher transportation demand (taxi activity) and arrest rates are associated with an increase in Uber pickups.
- event_count (0.37) and number_of_crashes (0.37): Both events and traffic incidents appear linked to increased Uber demand, likely due to congestion or disrupted transportation options, encouraging people to choose Uber.

* Low Correlations:
- wind_speed (-0.03) and number_of_persons_killed (0.01): These features show very weak correlations with Uber pickups, indicating minimal impact on demand. They are likely not useful predictors for the model.
