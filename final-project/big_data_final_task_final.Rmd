...
------------
###### Holiday
Here we would like to visualize whether it's better to combine the three separate holiday variables into a single is_holiday variable. We will do it by comparing the Uber demand across these holidays as individual variables and as a combined variable.

```{r}
# Step 1: Create the 'is_holiday' variable
add_is_holiday <- function(data) {
  data <- data %>%
    mutate(is_holiday = ifelse(memorial_day == 1 | independence_day == 1 | labor_day == 1, 1, 0))
  return(data)
}
```

```{r}
# Function to plot Uber demand for individual holidays with non-holiday percentage included
plot_individual_holidays <- function(data, dataset_name, color) {
  
  # Calculate total pickups on holidays (memorial_day, independence_day, labor_day)
  total_holiday_pickups <- data %>%
    filter(memorial_day == 1 | independence_day == 1 | labor_day == 1) %>%
    summarise(total_pickups = sum(number_of_pickups)) %>%
    pull(total_pickups)
  
  # Summarise holiday and non-holiday pickups
  holiday_summary <- data %>%
    summarise(
      memorial_day = sum(number_of_pickups[memorial_day == 1]),
      independence_day = sum(number_of_pickups[independence_day == 1]),
      labor_day = sum(number_of_pickups[labor_day == 1])
    ) %>%
    gather(key = "Holiday", value = "Pickups", memorial_day, independence_day, labor_day) %>%
    mutate(Pickup_Percentage = (Pickups / total_holiday_pickups) * 100)  # Calculate percentage relative to total holiday pickups
  
  # Create the bar plot
  ggplot(holiday_summary, aes(x = Holiday, y = Pickup_Percentage, fill = Holiday)) +
    geom_bar(stat = "identity", color = "black", fill = color) +
    labs(
      title = paste("Uber Demand by Individual Holidays -", dataset_name),
      x = "Holiday",
      y = "Percentage of Total Holiday Uber Pickups (%)"
    ) +
    theme_minimal() +
...
plot_individual_holidays <- function(data, dataset_name, color) {
  ...
  # Calculate total holiday pickups and non-holiday pickups
  holiday_summary <- data %>%
    summarise(
      memorial_day = sum(number_of_pickups[memorial_day == 1]),
      independence_day = sum(number_of_pickups[independence_day == 1]),
      labor_day = sum(number_of_pickups[labor_day == 1]),
      non_holiday = sum(number_of_pickups[memorial_day == 0 & independence_day == 0 & labor_day == 0])  # Non-holiday pickups
    ) %>%
    gather(key = "Holiday", value = "Pickups", memorial_day, independence_day, labor_day, non_holiday) %>%
    mutate(Pickup_Percentage = (Pickups / sum(Pickups)) * 100)  # Total pickups (holiday + non-holiday)
  
  # Create the bar plot with the updated percentage calculations
  ggplot(holiday_summary, aes(x = Holiday, y = Pickup_Percentage, fill = Holiday)) +
    geom_bar(stat = "identity", color = "black", fill = color) +
    labs(
      title = paste("Uber Demand by Individual Holidays -", dataset_name),
...
```{r}
train_filtered_1000m_data <- add_is_holiday(train_filtered_1000m_data)

# Plot Uber demand for individual holidays
plot_individual_holidays(train_filtered_1000m_data, "1000m Data", "lightblue")

# Plot Uber demand for the combined is_holiday variable
plot_combined_holiday(train_filtered_1000m_data, "1000m Data", "lightblue")
```
